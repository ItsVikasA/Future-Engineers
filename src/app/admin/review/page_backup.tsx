...
export default function ContentReview() {
  ...
  const handleViewDocument = (fileUrl: string, fileName: string, title: string) => {
    try {
      ...
      // For Cloudinary URLs, ensure proper viewing parameters
      if (viewUrl.includes('cloudinary.com')) {
        // Remove any download-forcing parameters
        viewUrl = viewUrl.replace(/fl_attachment[^/,]*[,]?/g, '');
        viewUrl = viewUrl.replace(/,+/g, ',').replace(/,$/, '');
        
        // Add inline flag to prevent forced download
        if (viewUrl.includes('/upload/') && !viewUrl.includes('fl_inline')) {
          viewUrl = viewUrl.replace('/upload/', '/upload/fl_inline/');
        }
      }
      
      // Open in new tab for viewing (this should open PDF viewer, not download)
